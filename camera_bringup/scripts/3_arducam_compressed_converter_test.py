...
class ImageRepublisherNode(Node):
    def __init__(self):
        ...
        self.color_sub = self.create_subscription(CompressedImage, '/camRight/image_raw/compressed', self.color_callback, 1)
        self.color_sub = self.create_subscription(CompressedImage, '/image_raw/compressed', self.color_callback, 1)
        self.color_sub = self.create_subscription(CompressedImage, '/camBack/image_raw/compressed', self.color_callback, 1)
        # self.color_pub_front = self.create_publisher(Image, '/repub/camFront/image_raw', 10)
        self.color_pub_right = self.create_publisher(Image, '/repub/camRight/image_raw', 1)
        self.color_pub_left = self.create_publisher(Image, '/repub/camLeft/image_raw', 1)
        self.color_pub_back = self.create_publisher(Image, '/repub/camBack/image_raw', 1)
