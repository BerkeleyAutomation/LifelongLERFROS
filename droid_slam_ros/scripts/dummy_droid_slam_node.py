...
class DummyDroidNode(Node):
    def __init__(self):
        ...
        tstamps = []
        iter_times = []
        for (t, image, depth, intrinsics) in tqdm(self.image_stream(args.datapath, use_depth=True, stride=stride)):
        ...
        traj_est = droid.terminate(self.image_stream(args.datapath, use_depth=False, stride=stride))
